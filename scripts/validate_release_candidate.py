...
def validate(root, version):
    gates={}
    required=["gpt-project.yaml","project-status.yaml","architecture.yaml",
    ...
    ok=True
    for p in sorted((root/"evals"/"reference-projects").glob("*.yaml")):
    ...
    for p in list((root/"scripts").rglob("*.py"))+list((root/"tests").rglob("*.py")):
        ...
        except SyntaxError: compile_ok=False
...
def main():
    ...
    print(json.dumps(result,ensure_ascii=False,indent=2) if args.json else f"Release candidate: {result['result'].upper()}")
